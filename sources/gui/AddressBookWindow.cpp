...
AddressBookWindow::AddressBookWindow(QWidget *parent) : QWidget(parent), contactList(true)
{
	initThemes();
	...
	auto c1 = connect(&btnCreate, &QAbstractButton::clicked, this, &AddressBookWindow::eventAddEntry);
	auto c2 = connect(&btnRemove, &QAbstractButton::clicked, this, &AddressBookWindow::eventRemoveEntry);
	auto c3 = connect(&contactList, &QListWidget::itemClicked, this, &AddressBookWindow::eventContactSelected);
	auto c4 = connect(&btnSearch, &QAbstractButton::clicked, this, &AddressBookWindow::eventSearch);
	auto c5 = connect(&btnImport, &QAbstractButton::clicked, this, &AddressBookWindow::eventImportEntries);
	auto c6 = connect(&btnClose, &QAbstractButton::clicked, this, &AddressBookWindow::close);
	auto c7 = connect(&contactDetail, &ContactDetails::SaveCurrentRecord, this, &AddressBookWindow::eventSaveEntry);
	auto c8 = connect(&themeSelector, &QCheckBox::clicked, this, &AddressBookWindow::eventSwitchTheme);
...
void AddressBookWindow::eventLoadContacts(std::vector<std::string> &ctlist)
{
	for (auto str : ctlist)
	{
...
void  AddressBookWindow::eventContactSelected(QListWidgetItem* item)
{
	currentItem = item;
...
void AddressBookWindow::eventAddEntry()
{
    isEdited = false;
	contactDetail.ResetFields();
}

void AddressBookWindow::eventRemoveEntry()
{
    if(opsPtr)
    {
...
void AddressBookWindow::eventSaveEntry()
{
	Contact rec;
...
void AddressBookWindow::eventSearch()
{
	contactList.clear();
...
void AddressBookWindow::SetOpsHandler(ContactOperations* ptr)
{
    opsPtr = ptr;
}

void AddressBookWindow::showEvent(QShowEvent *ev)
{
...
void AddressBookWindow::closeEvent(QCloseEvent *ev)
{
	QWidget::closeEvent(ev);
}

void AddressBookWindow::eventImportEntries()
{
	QList<Contact> clist;
...
void AddressBookWindow::eventSwitchTheme()
{
